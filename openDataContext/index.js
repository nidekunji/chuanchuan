...
function drawUser(x, y, user, i, color0) {
    drawImage(`openDataContext/assets/imgItemBg.png`, x, y);
    if (i < 3) {
    ...
    if (i < 3) {
        drawImage(`openDataContext/assets/NO${i + 1}.png`, x + 20, y + 22);
    } else {
        drawTextImage(`${i + 1}`, x + 48, y + 22, 26, `openDataContext/assets/rankIndex/rankIndex_`, 'center');
    }
    drawImage(`openDataContext/assets/imgAvatarBg.png`, x + 104, y + 10);
    drawAvatar(user.avatarUrl, x + 105, y + 11, 30);
    drawText(utils.subNickName(user.nickname), x + 176, y + 32, 20, color0);
    const detail = user.detail;
    drawTextImage(`${detail.score}`, x + 386, y + 20, 20, `openDataContext/assets/score/score_`, 'right');
}
...
function draw(res){
  ...
  res.data.forEach((user) => {
      let itemData = user// user.KVDataList.find((item) => item.key === key)
      if(!itemData)return;
